    ...
    for ml_type in "${model_learning_types[@]}"; do
        ...
        for sampling in "${active_sampling[@]}"; do
            ...
            for n_data in "${n_datas[@]}"; do
                ...
                python "$script_dir/planning_edge.py" "$obj" "$model_type" "$learning_type" "$n_data" $sampling $predef_controls $n_reps &
                echo
            done
            wait
        done
...
printf "=== All planning jobs completed in %02dh %02dm %02ds ===\n" \
